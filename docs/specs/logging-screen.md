...
7. **Compact notation when all sets are identical.** Trainer writes `35×7×5` on paper for "5 sets of 7 at 35 lb." When every set has the same prescribed reps and weight, render the header as `name · weight × reps × sets` (e.g. `Press · 35 lb × 7 × 5`) and drop the per-set rep numbers — just show the circles. As soon as one set is logged with a different value, that cell switches to inline `○ ~~7~~ 6` and the rest stay compact. If the trainer wrote per-set variation in the program (DB 30×12,10,8), inline rep numbers stay from the start. Reduces visual noise on the most common case.
