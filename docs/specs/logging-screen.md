...
6. **Tap-target proximity between tally circle and rep number.** They sit ~4px apart on phone, which makes mistaps possible (observed in pilot use). Spacing them out far enough to fully prevent it would dilute the notebook aesthetic. Hold for now; revisit if the trainer's clients complain. Style wins over margin until we have evidence otherwise.
7. **Header shorthand when all sets are identical.** Trainer writes `35×7×5` on paper for "5 sets of 7 at 35 lb." When every set has the same prescribed reps and weight, the header reads `name · weight × reps × sets` (e.g. `Press · 35 lb × 7 × 5`). The per-set `circle reps` cells stay rendered exactly as before — every set is still individually editable so you can log `7, 7, 6, 4, 3` and see the strikethroughs. The shorthand is a context label, not a display optimization that hides the editable cells. Lifts with varying reps per set (DB 30×12,10,8) skip the shorthand and just show `name · weight`.
8. **Admin shouldn't see (or interact with) tally circles.** Tally completion is a *client* state — checking a circle means "I did this set." Admin viewing a program is designing the plan, not doing it. Admin should see the lift and the prescription, no check circles. Today the screen renders them for everyone; needs a role-conditional split.
9. **Admin needs to edit prescribed reps and prescribed weight for the lift as a whole.** Long-press already covers Add set / Remove set / Remove lift, but there's no way for admin to change "5 sets of 7" to "5 sets of 8" except by tapping individual rep numbers — and those are hidden behind the compact header shorthand (#7). Admin needs the header itself to be editable: tap `7` in `35 lb × 7 × 5` to change prescribed reps for all sets; tap `5` to change set count (re-uses Add/Remove); tap `35 lb` to change weight. Effectively, the header pieces become first-class admin edit targets.
10. **Historical (completed) sessions are read-only.** Spec §4.5 currently allows "corrective edits" on completed sessions. The trainer wants the opposite: once marked complete, the session is locked — both for clients and admin. Edits-to-history would happen via a separate explicit "edit logged session" admin action, not the normal tap. Reduces accidental edits and matches the paper-notebook feel (you don't go back and rewrite yesterday's page). Affects: tap-to-check, tap-to-edit-reps, tap-to-edit-weight all become no-ops on `completed` programs.
11. **Admin sees the last 1–2 sessions when publishing.** When admin is viewing a draft (or just-published) program, show a small read-only reference of the previous one or two completed sessions next to or above the program. The trainer needs to know "what did the client do last A-day?" while planning today. Spec §5 mentions this as an iPad-landscape v2 idea; trainer wants it on every device, every publish. Likely a collapsed strip below the top bar that expands to show the prior session's logged values. Specific UI TBD.
